...
DirectoryMonitor.prototype.scanDir = function(dir) {
    var self = this;
    var d = when.defer();
    var scan_start_time = new Date();
    var scanlimiter = new ConcurrencyLimiter(1);
    ...
    wrench.readdirRecursive(dir, function(error, curFiles) {
        if(error) {
            ...
            self.emit("scanfailed", dir, error);
            d.resolve();
        }
        if(curFiles) {
            curFiles.forEach(function(f) {
                scanlimiter.do(function() {
            ...
            d.resolve(scanlimiter.drain().then(function() {
                console.log("scan finished; finding stale files");
                return self.dirdb.findStale(dir, scan_start_time).then(function(){
                    console.log("finished looking for stale files");
                    self.emit("scancomplete", dir);
                    return true;
                });
            }));
        }
