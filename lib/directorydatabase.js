...
DirectoryDatabase.prototype.setup = function() {
    var sqls = [
        ...
        "CREATE INDEX IF NOT EXISTS subpath ON files (subpath)"
    ];
            ...
            self.db.query("SELECT id,stathash FROM files WHERE directory_id = $dir_id AND subpath_hash = $subpath_hash AND subpath = $subpath",
                {
                    ...
                    if(result.length == 0) {
                        ...
                        //fixme: this UPDATE could be chunked up and done in bulk, so long
                        //as there was a mechanism for #findStale to flush that queue before
                        //doing its business.
...
DirectoryDatabase.prototype.findStale = function(dir_path, min_date) {
    var ids = [];
    var paths = [];
    var self = this;
    return this.findDir(dir_path).then(function(dir) {
        return self.db.query("SELECT id, subpath FROM files WHERE directory_id = $dir_id AND statdate < $min_date",
            {
                $dir_id: dir.id,
                $min_date: min_date
            }
            ...
            for(var i=0; i<results.length; i++) {
                ...
                paths.push(results[i].subpath);
            }
            if(ids.length > 0) {
                return self.db.query("DELETE FROM files WHERE id IN ($ids)", {
                    $ids: ids.join(",")
                });
            }
